...
export const orderController = {
  ...
  getMine: asyncHandler(async (req, res) => sendSuccess(res, await orderService.getForUser(req.user!.id, req.params.id))),
  listAll: asyncHandler(async (_req, res) => sendSuccess(res, await orderService.listAll())),
  updateStatus: asyncHandler(async (req, res) => sendSuccess(res, await orderService.updateStatus(req.params.id, req.body.status), 'Order status updated'))
};
